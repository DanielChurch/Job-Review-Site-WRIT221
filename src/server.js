...
async function getCompanyInfoFromJobTitle(title) {
  let result = await fetch(
    `${api_url}/getCompanyInfoFromJobTitle?title=${title}`
  );
  return await result.json();
}
...
async function saveReview(
  ...
  await fetch(
    `${api_url}/addReview?name=${title}&location=${location}&content=${content}&user=${user}&likes=${likes}&time=${time}&icon=${icon}&email=${email}`
  );
}
